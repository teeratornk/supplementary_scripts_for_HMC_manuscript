...
from multiphenics.la import as_backend_type
...
class PETScTSIntegrator(object):
    def __init__(self, problem, solution, solution_dot):
        ...
        self.ts = PETSc.TS().create(solution.block_function_space().mesh().mpi_comm())
        ...
        for (key, value) in parameters.items():
            if key == "exact_final_time":
                ...
                if value == "linear":
                    ...
                    #snes.setType("anderson") #TODO
                    #-snes_anderson_beta
                else:
                    raise ValueError("Invalid paramater passed as problem type.")
            elif key == "report":
                if value is True:
                    def print_time_pre_step(ts):
                        t = ts.getTime()
                        dt = ts.getTimeStep()
                        print("# t = {0:g}".format(t + dt))
                    self.ts.setPreStep(print_time_pre_step)
                else:
                    def do_nothing(ts):
                        pass
    ...
    def solve(self):
        ...
        # Solve
        solution_copy = self.solution.copy(deepcopy=True) # create copy to avoid possible internal storage overwriting by linesearch
        petsc_solution_copy = as_backend_type(solution_copy).block_vector().vec()
...
class _Monitor(object):
    ...
    def init(self, solution, solution_dot):
        if self.monitor_callback is not None:
            self.monitor_solution = solution.copy(deepcopy=True)
            self.monitor_solution_prev = solution.copy(deepcopy=True)
            self.monitor_solution_dot = solution_dot.copy(deepcopy=True)
    ...
    def _evaluate_solution(self, t, result):
        ...
        if isclose(t, self.t0, atol=self.monitor_eps): # t = t0
            ...
            result_petsc = as_backend_type(result).block_vector().vec()
    ...
    def _evaluate_solution_dot(self, t, result):
        ...
        if isclose(t, self.t0, atol=self.monitor_eps): # t = t0
            ...
            #print(t, self.T)
            #print(t, current_dt, self.ts.getTime(), self.ts.getTimeStep())
            ...
            result_petsc = as_backend_type(result).block_vector().vec()
            result_petsc -= as_backend_type(self.monitor_solution_prev).block_vector().vec()
